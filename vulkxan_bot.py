...
async def health_server():
    """Render Free Web Service uchun health check server."""
    from aiohttp import web

    async def health(request):
        db = Database()
        running = db.get("running", "0") == "1"
        return web.Response(
            text=json.dumps({
                "status": "ok",
                "bot": "vulkxan",
...
async def main_async():
    """Asosiy async entry point."""
    # Health server (Render Free uchun)
    runner = await health_server()

    try:
        bot = VulkxanBot()
        await bot.app_run_async()
    finally:
        await runner.cleanup()


class VulkxanBotAsync(VulkxanBot):
    async def app_run_async(self):
        """Async run for Web Service."""
        if not Config.TELEGRAM_TOKEN:
            log.error("TELEGRAM_TOKEN yo'q!")
            return
        if not Config.OWNER_CHAT_ID:
